//
//  RegionConfiguration+CoreDataProperties.swift
//  
//
...
extension RegionConfiguration {

    @nonobjc public class func fetchRequest() -> NSFetchRequest<RegionConfiguration> {
        return NSFetchRequest<RegionConfiguration>(entityName: "RegionConfiguration")
    }
    ...
    @NSManaged public var events: Int16s?

}

extension RegionConfiguration {
  func asRegionConfigurationModel() -> RegionConfigurationModel {
    var eventProtocols = [EventProtocol]()
    if let events = events {
      eventProtocols = events.arrayOfInts.compactMap{(EventId.init(rawValue: $0) ?? EventId.defaultValue).asEventType().asEventProtocol()}
    }
      
    return RegionConfigurationModel(id: self.id ?? "", latitude: self.latitude, longitude: self.longitude, radius: self.radius, title: self.title ?? "", events: eventProtocols)
  }
}
