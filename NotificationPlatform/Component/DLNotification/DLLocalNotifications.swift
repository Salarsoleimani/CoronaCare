...
public class DLNotificationScheduler {
  ...
  fileprivate func scheduleNotificationInternal ( notification: DLNotification) -> String? {
    ...
    if notification.scheduled {
      ...
      if !(notification.badge == nil) { content.badge = NSNumber(value: notification.badge!) }
      
      if !(notification.userInfo == nil) { content.userInfo = notification.userInfo! }
  ...
  public func repeatsFromToDate (identifier: String, alertTitle: String, alertBody: String, fromDate: Date, toDate: Date, interval: Double, repeats: RepeatingInterval, category: String = " ", sound: String = " ", repeatCount: Int) {
    ...
    var intervalDifference = Int( toDate.timeIntervalSince(fromDate) / (interval <= 0 ? 1 : interval) )
    intervalDifference = intervalDifference <= 0 ? 0 : intervalDifference
    ...
    for i in 0..<intervalDifference {
      ...
      notification.category = category
      notification.soundName = sound
      notification.userInfo = ["repeatCount": "\(repeatCount)"]

      do {
        guard let imageUrl = Bundle.main.url(forResource: category, withExtension: "png") else { return }
